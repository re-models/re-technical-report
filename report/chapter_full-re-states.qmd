...
An RE model is not required to yield a full RE state in every case. However, from the viewpoint of model evaluation, it is still desirable to have a model that is somewhat likely to reach full RE states. This is especially relevant to the fixed points of locally optimizing model variants, which have a severely restricted set of options at every adjustment step.
