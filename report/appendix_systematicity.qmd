...
More formally, let $S_{\mathcal{T}}$ be the domain of $\mathcal{T}$, and let $S\setminus\mathcal{T}=S\setminus S_{\mathcal{T}}$ be the domain outside the principles of $\mathcal{T}$, $2m$ the size of the restricted sentence pool and $\sigma_{\mathcal{T}}^{S\setminus\mathcal{T}}$ the number of positions that are dialectically consistent given $\mathcal{T}$ and complete on the restricted domain $S\setminus\mathcal{T}$. Then, we can define the *effective content size* on $S\setminus\mathcal{T}$ as
