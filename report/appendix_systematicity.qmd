...
By following vertical lines (with a constant size of theories' closure), you can see that everything else being equal, "smaller" theories receive higher systematicity values. Hence, $S_{BBB}$ satisfies *D2 (simplicity)*. By following the plotted lines (constant theory size), you can see that $S_{BBB}$ satisfies *D1 (content)* for all theory sizes except for $\vert \mathcal{T} \vert = 1$. As noted before (see @sec-extreme-values), these singleton theories receive maximum systematicity independent of their content. 

How problematic is this violation of *D1*? After all, *D1* is only violated for singleton theories and only violated in a "weak" sense. While it is true that systematicity does not *monotonically* increase with increasing content for singleton theories, systematicity does at least not decrease with increasing content. In @sec-appendix-trivial, we presented a preliminary analysis of how pervasive singleton theories are in fixed points and global optima. Their sparse emergence suggests that singleton theories (with low content) do not have a significant advantage over other theories. But that does not mean that the violation of *D1* does not cause trouble in other contexts. We should, therefore, consider and analyze other systematicity measures, which we intend in this appendix.

### D3 -- Minimal Systematicity

Besides, there are other constraints as well. Due to the finiteness of the sentence pool, there are lower bounds and upper bounds for systematicity. The systematicity measure $S_{BBB}$ is normalized to yield values within the unit interval $[0,1]$. We will follow this convention.
...
**Minimal systematicity (D3)**: Systematicity should be minimal if a theory does not imply anything besides its principles (let's call them *empty theories*).

The formulation *D3* is, however, imprecise or even ambiguous. If we read it strongly, we might require:

**Minimal systematicity (D3.1)**: Theories that do not imply anything besides their principles (empty theories) receive lower systematicity values than other theories.

In other words, the systematicity values of empty theories are lower bounds for non-empty theories. One way of satisfying *D3.1* is to let $S(\mathcal{T})=0$ if $\mathcal{T}$ is an empty theory. But there are other possibilities. In particular, *D3.1* allows it to distribute different systematicity values to empty theories.

The measure $S_{BBB}$ does not satisfy *D3.1*---not only because of its preferential treatment of singleton theories. In the following, we will refer to points in figures such as @fig-s-bbb by using tuples of the form $(\vert \mathcal{T}\vert, \vert \overline{\mathcal{T}}\vert)$. For instance, the point $(3,4)$ denotes the equivalence class of theories of size three with a dialectical closure of size four. Empty theories are points of the form $(n,n)$, which are the left (lower) end points of lines in @fig-s-bbb. You can see in this figure that for $2n=14$, there are only four theories that are lower bounds for non-empty theories (namely, $(4,4)$, $(5,5)$, $(6,6)$, $(7,7)$). For the other empty theories, we can find non-empty theories that receive lower systematicity values (e.g., $S_{BBB}(3,3)>S_{BBB}(6,7)$). Hence, $S_{BBB}$ violates *D3.1*. 

There is, however, a weaker interpretation of *D3*. We might only demand that empty theories receive the lowest systematicity value in comparison to non-empty theories with the same amount of principles (e.g., $S(3,3) < S(3,4) < \dots < S(3,n)$). This weaker criterion is satisfied by $S_{BBB}$. Since this weak version of *D3* is already implied by *D1 (content)*, we will not list it as an additional criterion.

If empty theories are, in some sense, the least systematizing, we might ask which theories are most systematizing. According to the above-formulated intuitions, we might suggest that theories with the least number of principles and the largest number of implications should receive maximum systematicity values. For a sentence pool of size $14$, these are singleton theories that imply seven sentences. Similar to the weak version of *D3*, this criterion is satisfied by $S_{BBB}$ and already implied by *D1* and *D2*. 
...
However, that might be too strong since one might want to satisfy *D3.1* by letting $S=0$ for empty theories. But then, one cannot further reduce systematicity for ad hoc extensions of empty theories. Hence, *D.3* might conflict with the requirement that systematicity should decrease with ad hoc extension. Fortunately, there is a simple solution. The described case is only relevant for non-empty theories. Hence, an appropriate strengthening of *D4* is:

**D4.1 (non-ad-hocness)**: Extending a non-empty theory with ad hoc principles should (monotonically) decrease its systematicity; extending an empty theory with ad hoc principles must not increase its systematicity.
...
One rationale for *D4 (non-ad-hocness)* was the intuition that ad hoc principles are loners in some way. They do not work together with other principles in implying other sentences than the theories' principles; they do not add something to the inferential potential of a theory besides themselves. The requirement *D4* is, therefore, a special case of a more general requirement that demands:
...
Consider the dialectical structure depicted in @fig-map-sys1 and the theories $\mathcal{T}_1 = \{1,2\}$ and $\mathcal{T}_2 = \{7,8\}$. Both theories have the same size ($2$) and the same size of their dialectical closure ($6$). The principles of $\mathcal{T}_1$ work in the following sense together: We need both principles to deduce the other sentences of its dialectical closure ($3-6$). In contrast, the principles of $\mathcal{T}_2$ do not work together. Instead, the inferential workload is distributed among its principles: The principle $7$ implies $3$ and $4$, and the other ($8$) implies $5$ and $6$. According to *D5*, we should expect that $S(\mathcal{T}_2) < S(\mathcal{T}_1)$
...
A similar case is depicted in @fig-map-sys2. Here, you do not need all principles of the theory $\mathcal{T}_1 = \{1,2,4\}$ to deduce the sentences $3$ and $4$, but they still work together since you need sentence $2$. In contrast, the principles of the theory $\mathcal{T}_2 = \{1,6,7\}$ work alone to deduce $3$ and $4$. (Here, $1$ is even an ad hoc principle.) Again, *D5* requires that $S(\mathcal{T}_2) < S(\mathcal{T}_1)$
...
For instance, the theory  $\mathcal{T}_1=\{1\}$ does not imply anything on its own (besides its principle) and is thus on par with other singleton theories. However, in contrast to, let's say, the theory $\mathcal{T}_2=\{4\}$, it does imply sentences if it is combined with other sentences. We might, therefore, expect that the systematicity of $\mathcal{T}_1$ is higher than the systematicity of $\mathcal{T}_2$. 
...
The measure $S_{BBB}$ violates *D1 (content)* due to the nominator $\vert \mathcal{T}\vert-1$ in @eq-s-bbb, which becomes zero for singleton theories. Accordingly, singleton theories receive maximum systematicity independent of their content. One simple suggestion to fix this behaviour is to adapt the nominator such that it does not become zero for theories of size one. A minimal adaption would be to subtract smaller values than one:
...
@fig-sys-mm plots the systematicity measure for different values of the parameter $\gamma$. By construction, the measure satisfies *D1 (content)*. Similar to $S_{BBB}$, it also satisfies *D2 (simplicity)* and *D4.1 (non-ad-hocness)*. It is even possible to comply with *D3.1 (minimal systematicity)* if we set $\gamma$ high enough. In our case (sentence pool of size *14*), $\gamma=0.1$ is able to push the systematicity values of $(m,m)$ theories (i.e., empty theories) such that they are lower bounds for non-empty theories. 
...
Maximally systematizing theories are singleton theories that are able to imply for every sentence $s$ outside the domain of theory either $s$ or its negation.^[If $\mathcal{S}$ is the sentence pool, the domain of a theory $\mathcal{T}$ is defined by $\{s\in \mathcal{S}\vert s\in\mathcal{S}\text{ or }\neg s\in\mathcal{T}\}$.] For such theories, we have $\vert \overline{\mathcal{T}}\vert - \vert \mathcal{T}\vert = n-1$, which motivates the denominator. Worst cases are empty theories for which $\vert \overline{\mathcal{T}}\vert = \vert \mathcal{T}\vert$ holds, which yields $S_{ec}=0$.
...
#plot_sys_measure("s_sligs", scale=0.8)
plot_sys_measure(sys_data, "sys_ec")
plt.show()

#plot_sys_measure("s_squags", scale=0.8)
...
- Scope penalties: $n - \vert\overline{\mathcal{T}}\vert$
...
Similarly to $S_{ec}$, the new measure $S_{csw_\alpha}$ complies with *D3.1 (minimal systematicity)*. They do so in a very specific way: The systematicity values for empty theories are not only lower bounds for non-empty theories, but they also receive the same and lowest systematicity value possible, namely $0$. 
...
This suggests to measure $\frac{\vert \overline{\mathcal{T}}\vert - \vert \mathcal{T}\vert}{\vert \mathcal{T}\vert}$ instead of simply measuring $\vert \overline{\mathcal{T}}\vert - \vert \mathcal{T}\vert$: E.g., sth. like:
...
Alternatively, we can conceptualise $\frac{\vert \overline{\mathcal{T}}\vert - \vert \mathcal{T}\vert}{\vert \mathcal{T}\vert}$ as a multiplicative correction factor for $S_{ec}$ which leads to sth. like 
...
A *complete position* is a position that includes for each $s\in\mathcal{S}$ either $s$ or $\neg s$. If a dialectical structure is empty and thusly doesn't render any position inconsistent, then there are $2^n$ complete and consistent positions (we assume that complete positions do not include flat contradictions, i.e., are minimally consistent). In this case, $D(\tau)=0$. If, on the other hand, $\tau$ implies one complete position (and, hence, renders all other positions as dialectically inconsistent), $D(\tau)=1$.  
...
There are surely different possibilities to introduce systematicity measures based on the ($\sigma$-based) content size $\vert C_\sigma(\mathcal{T})\vert$. Here, we discuss but one measure based on the same considerations we used to devise the measure $S_{rec}$ (@sec-rel-eff-content-systematicity). The basic idea of this measure was to take the effective content size $\vert\overline{\mathcal{T}}\setminus\mathcal{T}\vert$ in relation to the size of the theory.
...
More formally, let $S_{\mathcal{T}}$ be the domain of $\mathcal{T}$, $S\setminus\mathcal{T}=S\setminus S_{\mathcal{T}}$ the domain outside the principles of $\mathcal{T}$, $2m$ the size of the restricted sentence pool and $\sigma_{X}^{S\setminus\mathcal{T}}$ the number of positions that are dialectically consistent given $\mathcal{T}$ and complete on the restricted domain $S\setminus\mathcal{T}$, then we can define the *effective content size* on $S\setminus\mathcal{T}$ as:

$$
\vert C_\sigma(\mathcal{T, S\setminus\mathcal{T}})\vert = \frac{lg(\sigma^{S\setminus\mathcal{T}}-\sigma^{S\setminus\mathcal{T}}_{\mathcal{T}}+1)}{m}
...
$$
S\propto\frac{lg(\sigma^{S\setminus\mathcal{T}}-\sigma^{S\setminus\mathcal{T}}_{\mathcal{T}}+1)}{\vert\mathcal{T}\vert \cdot m}
$$ 

What remains is a proper normalization. 

Maximal effective content is reached by singleton theories that render all but one position inconsistent. Under this assumption we have $\sigma^{S\setminus\mathcal{T}}_{\mathcal{T}}=1$ and $\vert \mathcal{T}\vert=1$. The value of $\sigma^{S\setminus\mathcal{T}}$ will depend on the singleton theory in question. One suggestion is, therefore, to take $max(\{\sigma^{S\setminus \{s\}})\vert s\in S \}$ to normalize the measure. 

For simplicity, we will use another estimation. $\sigma^{S\setminus\mathcal{T}}$ will be maximal if the dialectical structure is silent on the domain $S\setminus\mathcal{T}$; that is, if it doesn't render anything dialectically inconsistent on $S\setminus\mathcal{T}$. In this case, $\sigma^{S\setminus\mathcal{T}}$ will be $2^{n-1}$ for singleton theories. This motivates the following normalization:^[Admittedly, $S_{grec}$ will under this construction never reach the value one, because $\sigma^{S\setminus\mathcal{T}}=2^{n-1}$ means that the theory won't imply anything on $S\setminus\mathcal{T}$. However, $S_{grec}$ will still have maximal systematicity values for singleton theories that have maximal content on $S\setminus\mathcal{T}$.]

$$
S_{grec}(\mathcal{T})=\frac{lg(\sigma^{S\setminus\mathcal{T}}-\sigma^{S\setminus\mathcal{T}}_{\mathcal{T}}+1)}{\vert\mathcal{T}\vert \cdot (n-1)}
$$ {#eq-s-grec}
...
The desiderata *D1 (content)* and *D2 (simplicity)* are trivially satisfied. The nominator of @eq-s-grec is proportional to the size of the generalized content; the denominator is proportional to the theory size. Accordingly, if we keep the size of the theory constant, systematicity increases with increasing content (*D1*). Similarly, if we keep the theory's content constant, systematicity increases with a decrease in theory size (*D2*). Figure @fig-sys-grec illustrates this behaviour.^[The figure is plotted based on a data set of 100 randomly generated dialectical structures and all possible theory candidates for each $\tau$. This dataset is not needed to plot the function $S_{grec}$. However, it contains all information of $\tau$-theory pairs to assess $\sigma$-based systematicity measures in detail (e.g., it contains for each pair $\sigma^{S\setminus\mathcal{T}}$ and $\sigma^{S\setminus\mathcal{T}}_{\mathcal{T}}$).]
...
Empty theories do not imply anything besides their principles that is not already tautologically true (with respect to $\tau$). Accordingly, they do not render any positions inconsistent on $\sigma^{S\setminus\mathcal{T}}$ that are not already dialectically inconsistent according to $\tau$ alone. Hence, we have $\sigma^{S\setminus\mathcal{T}}=\sigma^{S\setminus\mathcal{T}}_{\mathcal{T}}$, which implies that $S_{grec}(\mathcal{T})=0$ for empty theories. Non-empty theories, on the other hand, do imply something additional on $\sigma^{S\setminus\mathcal{T}}$. Hence, we have  $\sigma^{S\setminus\mathcal{T}}>\sigma^{S\setminus\mathcal{T}}_{\mathcal{T}}$ and accordingly $S_{grec}(\mathcal{T})>0$ for non-empty theories. Taken together, this implies that systematicity values of empty theories are lower bounds for those of non-empty theories (*D3.1*).
...
$$
\frac{lg(\sigma^{S\setminus\mathcal{T}^*}-\sigma^{S\setminus\mathcal{T}^*}_{\mathcal{T}^*}+1)}{\vert\mathcal{T}^{*}\vert \cdot (n-1)} < \frac{lg(\sigma^{S\setminus\mathcal{T}}-\sigma^{S\setminus\mathcal{T}}_{\mathcal{T}}+1)}{\vert\mathcal{T}\vert \cdot (n-1)}
$$
...
Basically, we have to compare $\sigma^{S\setminus\mathcal{T}^*}-\sigma^{S\setminus\mathcal{T}^*}_{\mathcal{T}^*}$ with $\sigma^{S\setminus\mathcal{T}}-\sigma^{S\setminus\mathcal{T}}_{\mathcal{T}}$. Clearly, $\sigma^{S\setminus\mathcal{T}^*}\leq\sigma^{S\setminus\mathcal{T}}$ (since $S\setminus\mathcal{T}^*\subset S\setminus\mathcal{T}$). Additionally, we can use the definition of ad hoc principles: Adding ad hoc principles to a theory $\mathcal{T}$ does not do anything in addition to $\mathcal{T}$ on $S\setminus\mathcal{T}^*$. Hence, $\sigma^{S\setminus\mathcal{T}^*}_{\mathcal{T}^*}=\sigma^{S\setminus\mathcal{T}^*}_{\mathcal{T}}$. Considering this equation, we have to compare $\sigma^{S\setminus\mathcal{T}^*}_{\mathcal{T}}$ with $\sigma^{S\setminus\mathcal{T}}_{\mathcal{T}}$ and, again, we have $\sigma^{S\setminus\mathcal{T}^*}_{\mathcal{T}}\leq \sigma^{S\setminus\mathcal{T}}_{\mathcal{T}}$. But this simple estimation does not help to get us any further with comparing $\sigma^{S\setminus\mathcal{T}^*}-\sigma^{S\setminus\mathcal{T}^*}_{\mathcal{T}^*}$ and $\sigma^{S\setminus\mathcal{T}}-\sigma^{S\setminus\mathcal{T}}_{\mathcal{T}}$. 

What we need is a more precise estimation of the forms $\sigma^{S\setminus\mathcal{T}^*}+a = \sigma^{S\setminus\mathcal{T}}$ and $\sigma^{S\setminus\mathcal{T}^*}_{\mathcal{T}}+b= \sigma^{S\setminus\mathcal{T}}_{\mathcal{T}}$. In other words, we need to know the extent to which $\sigma^{S\setminus\mathcal{T}}_{(\mathcal{T})}$ increases when further restricting the sentence pool.

Let's start with $\sigma^{S\setminus\mathcal{T}^*}$ and $\sigma^{S\setminus\mathcal{T}}$. We will search for an additive expression for both terms. Let $\Gamma$ be the set of all complete and consistent positions on $\tau$ (hence, $\sigma=\vert \Gamma\vert$). In analogy to $\sigma^{\mathcal{S}'}$, we will define $\Gamma^{\mathcal{S}'}$ as the set of consistent positions that are complete on the subdomain $\mathcal{S}'\subset \mathcal{S}$. More formally, we can define:
...
$\Gamma^{\mathcal{S}\setminus\mathcal{T}}$ can be partioned into 
$$
\Gamma^{\mathcal{S}\setminus\mathcal{T}}= \Gamma^{\mathcal{S}\setminus\mathcal{T}}_{\{p\}} \cup \Gamma^{\mathcal{S}\setminus\mathcal{T}}_{\{\neg p\}} 
$$

we can use the above defined sets to rewrite $\Gamma^{\mathcal{S}\setminus\mathcal{T}}_{\{p\}}$ and $\Gamma^{\mathcal{S}\setminus\mathcal{T}}_{\{\neg p\}}$ in the following way:
