...
::: {.callout-note}
The results of this chapter can be reproduced with the Jupyter notebook located [here](https://github.com/debatelab/re-technical-report/blob/main/notebooks/data_analysis_appendix_trivial_endpoints.ipynb).
:::
...
- Overall, the relative share of trivial gobal optima (@tbl-trivial_go) and fixed points (result perspective @tbl-trivial_fp_rp) is very low for quadratic model variants
- Linear model variants exhibit substantially more trivial global optima, but the relative shares are still low.
- `LinearLocalRE` exhibits a substantial share of trivial fixed points in the process perspective (@tbl-trivial_fp_pp), but not for the result perspectve (@tbl-trivial_fp_rp).  This indicates that relatively many branches lead to trivial fixed points.
