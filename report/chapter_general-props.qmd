...
For `LinearGlobalRE` there are, again, islands with many branches (the cells $(0.4,0.3)$, $(0.6,0.2)$ and $(0.8,0.1)$) which are characterised by $\alpha_F = \alpha_A$. The high number of branches correlates with a high number of global optima within these cells (compare @fig-hm-mean-n-global-optima). It stands, therefore, to reason that the large amount of branches can be explained by a large amount of global optima that are reached by RE processes (see XXX in XXX). 

The "$\alpha_F=\alpha_A$"-line is also relevant for the `LinearLocalRE` model. Above that line, branching is comparably low (roughly $1-3$) and below that line comparably high (with a high variance).  This behaviour is to some extent even observable in the `QuadraticLocalRE` model.
