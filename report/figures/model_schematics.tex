...
\begin{document}
\begin{tikzpicture}
	\rbox{RE coms}{}{commitments\\$\mathcal{C}_{i}$}
	\rbox{RE the}{right=4cm of RE coms} {theory\\$\mathcal{T}_{i}$}
	
	\draw[<->, line width=2pt, shorten >=5pt, shorten <=5pt, color=Prism3] (RE coms) -- (RE the) node[midway, below] (fit) { $\alpha_{A}\cdot A(\mathcal{C}_{i}, \mathcal{T}_{i})$} node[midway, above] (account) {account};
	
	\group{RE pos}{RE coms}{RE the}{epistemic state $(\mathcal{C}_{i}, \mathcal{T}_{i})$}
	...
	node[pos=0.5, below] (just) {$\alpha_{S}\cdot S(\mathcal{T}_{i})$} node[pos=0.5, above] (sys) {systematicity};
	
	\node (in) [below=4.7cm of RE coms] {};
	
	\rbox{inbox}{below=4.7cm of RE coms}{input\\ commitments\\$\mathcal{C}_{0}$}

	\grbox{com1}{above=0.3cm of inbox}{commitments}
	\grbox{the1}{at={(RE the |- inbox)}}{theory\\$\mathcal{T}_{1}$}
	...
	\draw[->, line width=2pt, shorten >=2pt, color=Prism3] (1.center)  |- (inbox) node[pos=0.25, left] (resp) {\begin{tabular}{r}
	     faithfulness\\
	     $\alpha_{F}\cdot F(\mathcal{C}_{i}\,\vert\, \mathcal{C}_{0})$ 
	\end{tabular}};
\end{tikzpicture}
\end{document}