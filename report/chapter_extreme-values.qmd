...
Hence, $S(\mathcal{T}) = 1$ if and only if $\vert \mathcal{T}\vert = 1$ (i.e., if and only if $\mathcal{T}$ is a singleton theory, e.g., $\mathcal{T} =\lbrace s\rbrace$). Note that it does not matter whether $G$ is linear or quadratic. Furthermore, $S(T) = 0$ if and only if $\mathcal{T}=\emptyset$ by definition.
